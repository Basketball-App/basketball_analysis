...
class BallAcquisitionDetector:
    def __init__(self):
        self.posession_threshold = 50
        self.min_ball_frames = 12
    ...
    def min_distance_to_ball(self, ball_center, player_bbox):
    ...
    def find_best_candidate_for_possession(self, ball_center, player_tracks_frame, ball_bbox):
        ...
        for player_id, player_info in player_tracks_frame.items():
            ...
            min_distance_between_player_ball = self.min_distance_to_ball(ball_center, player_bbox)

            if containment > self.containment_threshold:
                high_containment_players.append((player_id, containment))
            else:
                regular_distance_players.append((player_id, min_distance_between_player_ball))
        ...
        if regular_distance_players:
            ...
            if best_candidate[1] < self.posession_threshold:
                return best_candidate[0]
    ...
    def detect_ball_possession(self, player_tracks, ball_tracks):
        ...
        for frame_num in range(num_frames):
            ...
            if best_player_id != -1:
                ...
                consecutive_possesion_count = {best_player_id: number_of_consecutive_frames}

                if consecutive_possesion_count[best_player_id] >= self.min_ball_frames:
                    possessions[frame_num] = best_player_id
